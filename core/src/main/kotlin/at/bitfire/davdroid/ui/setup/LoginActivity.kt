            ...
            val loginInfoState = produceState<LoginInfo?>(initialValue = null) {
                withContext(Dispatchers.Default) {
                    value = if (intent.extras?.getString("code") != null) {
                        ...
                        val infomaniakPassword = intent.extras?.getString("infomaniakPassword")?.toSensitiveString()
                        LoginInfo(
                            baseUri = URI(SYNC_INFOMANIAK),
                            credentials = Credentials(username = intent.extras?.getString("infomaniakLogin"), password = infomaniakPassword),
                        )
                    }
                }
            }
    ...
    companion object {
        ...
        private suspend fun getCredentials(code: String, infomaniakLogin: InfomaniakLogin): Credentials? {
            try {
                ...
                val credentials = Credentials(infomaniakUser.login, infomaniakPassword.password?.toSensitiveString())
